...
namespace Emperia.Buffs
{
	public class CursedSkull : ModBuff
	{
		public override void SetDefaults()
		{
			...
			Description.SetDefault("His eyes glow and make light");
		...
		{
			player.GetModPlayer<MyPlayer>(mod).skullLightPet = true;
			...
			if ((player.controlDown && player.releaseDown))
			{
				if (player.doubleTapCardinalTimer[0] > 0 && player.doubleTapCardinalTimer[0] != 15)
				{
					...
					{
						if (Main.projectile[j].active && Main.projectile[j].type == mod.ProjectileType("CursedSkull") && Main.projectile[j].owner == player.whoAmI)
						{
